...
<div class="app-main wide" id="committeesMain" style="display:none">
  ...
  <div id="cmDetailView" style="display:none">
    ...
    <div class="panel">
      <h3>الأعضاء</h3>
      <div class="search-row"><input type="text" id="cmMemberSearch" placeholder="ابحثي عن منتسبة لإضافتها…"></div>
    ...
    <div class="panel">
      ...
      <button class="btn gold" id="cmMinuteAdd" style="width:auto;padding:9px 20px;margin-top:10px">حفظ المحضر</button>
...
async function initCommittees(){
  ...
  $('cmMemberSearch').addEventListener('input',()=>{
    ...
    deb=setTimeout(async ()=>{
      ...
      box.querySelectorAll('div').forEach((el,i)=>el.addEventListener('click',()=>addMember(st[i])));
    },250);
  });
...
async function createCommittee(){
  ...
  try{
    const {data:committee,error}=await db.from('committees').insert({
      academic_year_id:S.YEAR.id, name, home_project_id:homeProjectId, head_staff_id:S.ME.id
    }).select('id').single();
...
async function loadCommittees(){
  const {data,error}=await db.from('committees')
    .select('id,name,home_project_id, plan_projects(name), committee_beneficiary_projects(plan_projects(name))')
  ...
  $('cmList').innerHTML=data.map(c=>{
    ...
    return `<div class="cm-row" data-id="${c.id}">
      <span><b>${c.name}</b><br><small style="color:#8a93a0">البيت: ${c.plan_projects?.name||'—'}${benef.length?` — مستفيدة أيضاً: ${benef.join('، ')}`:''}</small></span>
...
async function openCommittee(id){
  const {data:c}=await db.from('committees').select('id,name, plan_projects(name)').eq('id',id).single();
  if(!c) return;
  CUR_COMMITTEE=c;
  $('cmListView').style.display='none'; $('cmDetailView').style.display='block';
  $('cmDetailName').textContent=c.name;
  $('cmDetailMeta').textContent=`المشروع الأم: ${c.plan_projects?.name||'—'}`;
  await loadMembers(); await loadTasks(); await loadMinutes();
}

async function loadMembers(){
  const {data}=await db.from('committee_members').select('id,staff_id,is_head, staff(full_name)').eq('committee_id',CUR_COMMITTEE.id);
  const members=data||[];
  $('cmMembersList').innerHTML=members.length ? members.map(m=>`
    <div class="cm-row" style="cursor:default"><span>${m.staff?.full_name||'—'}${m.is_head?' <span class="cm-tag">رئيسة</span>':''}</span>
      <button class="btn ghost" data-id="${m.id}" style="width:auto;padding:6px 14px;font-size:12px;color:var(--err);border-color:var(--err)">✕ إزالة</button></div>`).join('')
    : '<div class="empty-day">لا أعضاء بعد.</div>';
...
async function addMember(staff){
  $('cmMemberSugg').style.display='none'; $('cmMemberSearch').value='';
  const {error}=await db.from('committee_members').insert({committee_id:CUR_COMMITTEE.id, staff_id:staff.id});
