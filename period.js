/* period.js — تقرير فترة زمنية للغياب (مثلاً شهر كامل)
   يولّد: صفحة ملخص (إجمالي الغياب يومياً، أكثر/أقل الصفوف غياباً،
   طالبات تجاوزن ٢٥٪ من أيام الفترة) + صفحة لكل يوم دراسي.
   يُحتسب "اليوم الدراسي" كل يوم أحد-خميس وُجد له رصد فعلي؛ الأيام التي
   لا يوجد لها أي رصد (عطلة غالباً) تُستبعد وتُعرض في قائمة منفصلة للتحقق.
...
const HIGH_THRESHOLD = 25; // % — عتبة «تجاوز الغياب» — قابلة للتعديل لاحقاً من الإعدادات
...
<style>
  ...
  #periodMain input[type=date]{padding:8px 10px;border:1.5px solid var(--line);border-radius:8px;font:inherit;background:var(--white)}
...
async function generateReport(){
  ...
  try{
    ...
    DAILY=results.filter(r=>!r.excluded);
    EXCLUDED=results.filter(r=>r.excluded);
    if(!DAILY.length){ toast('لا أيام دراسية صالحة في هذه الفترة'); return; }
    const schoolDaysCount=DAILY.length;
    ...
    const highStudents=Object.keys(perStudentAbsDays)
      .map(sid=>({sid, ...STU_INFO[sid], absDays:perStudentAbsDays[sid], rate:perStudentAbsDays[sid]/schoolDaysCount*100}))
...
function renderSummary(){
  ...
  if(EXCLUDED.length){
    ...
    $('perExcluded').innerHTML=`⚠️ أيام استُبعدت من الاحتساب (${EXCLUDED.length}) لعدم وجود رصد — تحققي أنها عطلات فعلاً:<br>`+
      EXCLUDED.map(e=>`• ${e.date} — ${e.reason}`).join('<br>');
...
function exportPdf(){
  ...
  const dayPages=DAILY.map(day=>{
    const rows=Object.entries(day.bySid).map(([sid,sec],i)=>{
      const st=STU_INFO[sid]||{};
...
registerTab({id:'periodMain', label:'تقرير فترة', group:'attendance', groupLabel:'متابعة الغياب',
  show:f=>f.isAdmin||f.isSocial||f.isReg||f.isLead||f.isAttendanceLead, onOpen:initPicks});
