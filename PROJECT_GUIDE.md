...
المستخدَم حالياً — بدقة الدقيقة). ملاحظة: يوجد أيضاً
`compensatory_hours_requests`/`compensatory_hours_usage`/
`compensatory_hours_balance` (أسماء مشابهة جداً، مسافة بينها مصدر لبس
سهل) — هذه تبدو نسخة سابقة/بديلة غير مستخدمة فعلياً في أي ملف `.js`
حالي (تحققي بحث بالكود قبل الاعتماد على أيّ منها).
...
- جداول `compensatory_hours_*` (مفرد الشكل، أسماء طويلة) موجودة بقاعدة
  البيانات لكن غير مستخدَمة بأي ملف `.js` حالي — يبدو أنها بقايا محاولة
  سابقة؛ لا تحذفيها بلا تأكيد (قد تحمل بيانات تاريخية)، ولا تبنِ عليها
  ميزة جديدة بدون تحقق أولاً من كونها فعلاً مهجورة.
