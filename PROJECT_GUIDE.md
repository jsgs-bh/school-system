...
| `subsMain` | الاحتياط | `substitutes.js` | الأدمن / مسؤولة الاحتياط | تأمين حصص المعلمات الغائبات + طباعة جدول الاحتياط ليوم واحد (تفصيل كامل بالقسم 15) |
...
| `myCoverage` | حصص الاحتياط | `substitutes.js` (نفس ملف `subsMain`) — حصص اليوم المُسنَدة لها + سجل حصص احتياط سابقة |
...
- **مُنجَز**: قائمة `BACKUP_TABLES` في `data-reset.js` كانت تفوّت خمسة
  جداول حيّة (`comp_hours`, `comp_hours_usage`, `substitute_assignments`,
  `announcements`, `announcement_recipients`) — أُضيفت الآن، والنسخة
  الاحتياطية الكاملة تشمل كل جدول موجود فعلياً بقاعدة البيانات (تحقَّق
  ذلك بمقارنة القائمة بجدول `information_schema.tables` وقت الإصلاح).
  أي جدول جديد يُنشأ مستقبلاً لازم يُضاف هنا يدوياً بالمثل.
